...
git config --local --unset-all core.askpass || true
git config --global --unset-all core.askpass || true

# 2) Push with VS Code askpass env vars disabled for this command only
env -u GIT_ASKPASS \
    -u SSH_ASKPASS \
    -u VSCODE_GIT_ASKPASS_NODE \
    -u VSCODE_GIT_ASKPASS_MAIN \
    -u VSCODE_GIT_ASKPASS_EXTRA_ARGS \
    git -c credential.helper=store -c core.askpass= push -u origin main

  commit code : git config --local --unset-all filter.lfs.process 2>/dev/null; git config --local --unset-all filter.lfs.smudge 2>/dev/null; git config --local --unset-all filter.lfs.clean 2>/dev/null; git config --local --unset-all filter.lfs.required 2>/dev/null; rm -f .git/hooks/pre-push; git push -u origin main
